...
$CONTAINER_CMD env CC="$CC" CXX="$CXX" CUDAHOSTCXX="$CUDAHOSTCXX" \
  torchrun $TORCHRUN_ARGS $(dirname "$0")/../../../../../../../pretrain_gpt.py \
    ...
    --topk-adams-full-param-cpu-offload \
    --topk-adams-residual-cpu-offload \
    > >(tee -a llama2_1.8B_AdamS_d_${density}_${timestamp}.out) \
    2> >(tee -a llama2_1.8B_AdamS_d_${density}_${timestamp}.err >&2)
    # --log-interval 1 \
    # --log-memory-interval 1 \
    # --timing-log-level 2 \
    # --timing-log-option minmax \
